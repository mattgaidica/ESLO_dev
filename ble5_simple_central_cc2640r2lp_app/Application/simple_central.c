...
#define SC_EVT_ADV_REPORT_FULL     0x0B
...
#define DEFAULT_SCAN_DURATION                0 // 1 sec
...
#define SC_ROW_DATA          (TBM_ROW_APP + 7)
...
static void SimpleCentral_init(void) {
	...
	uint8_t bdAddr[6] = { 0x11, 0x11, 0x11, 0x11, 0x11, 0x11 }; // matt
...
#if (DEFAULT_DEV_DISC_BY_SVC_UUID == TRUE)
		if (SimpleCentral_findSvcUuid(SIMPLEPROFILE_SERV_UUID, pAdvRpt->pData,
						pAdvRpt->dataLen)) {
...
//			Display_printf(dispHandle, SC_ROW_DATA, 0, "%s",
//					Util_convertBytes2Str(pAdvRpt->pData, 0x1F));
		}
#else // !DEFAULT_DEV_DISC_BY_SVC_UUID
//		Display_printf(dispHandle, SC_ROW_NON_CONN, 0, "Discovered: %s",
//				Util_convertBdAddr2Str(pAdvRpt->addr));

		uint8_t broadByte = pAdvRpt->pData[EEG_FLAG_BYTE];
		if (lastAdv == 0xFF) {
			if (broadByte != 0xFF) {
				doStream = true;
			}
		} else {
			if (broadByte > lastAdv) {
				doStream = true;
			}
		}
		if (doStream) {
	...
	case SC_EVT_SCAN_ENABLED: {
		...
		const char initMsg[] = { 0x01, 0x02, 0x03 };
	...
	case SC_EVT_SCAN_DISABLED: {
		...
		UART_close(uart);
		dispHandle = Display_open(Display_Type_ANY, NULL);
		Display_printf(dispHandle, SC_ROW_NON_CONN, 0, "%d devices discovered",
				numReport);
	...
	case SC_EVT_ADV_REPORT_FULL: {
		GapScan_discardAdvReportList();
//		Display_printf(dispHandle, SC_ROW_ANY_CONN, 0, "Report Full");
	}
...
static void SimpleCentral_processGapMsg(gapEventHdr_t *pMsg) {
	...
	case GAP_DEVICE_INIT_DONE_EVENT: {
		...
		GapScan_setEventMask(
				GAP_EVT_SCAN_ENABLED | GAP_EVT_SCAN_DISABLED | GAP_EVT_ADV_REPORT); // rm: GAP_EVT_ADV_REPORT_FULL
		...
		temp8 = SCAN_FLT_DUP_ENABLE; // SCAN_FLT_DUP_DISABLE
		...
		GapScan_setParam(SCAN_PARAM_FLT_POLICY, &temp8);
...
void SimpleCentral_scanCb(uint32_t evt, void *pMsg, uintptr_t arg) {
	...
	} else if (evt & GAP_EVT_ADV_REPORT_FULL) {
		event = SC_EVT_ADV_REPORT_FULL;
	} else {
		return;
	}
